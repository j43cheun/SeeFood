...
struct SeeFoodView: View {
    ...
    private var navigationBarTitle: String {
        ...
        if self.classifier.label != nil {
            title = self.classifier.label!.replacingOccurrences(of: "_", with: " ")
        }
        ...
        VStack {
            if self.classifier.isProcessing {
                ...
                if let image = self.classifier.image {
                    ...
                    Text("Press the button below to identify food")
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                }
            }
                ...
                List {
                    if let classifications = self.classifier.classifications {
                        ForEach(classifications.indices) { index in
                            HStack {
                                Text(classifications[index].identifier.replacingOccurrences(of: "_", with: " "))
                                Spacer()
